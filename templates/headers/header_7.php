...
if (!function_exists('logistic_company_template_header_7_output'))
  ...
  function logistic_company_template_header_7_output($post_options, $post_data)
    ...
    $header_css = '';
    if (empty($header_image)) $header_image = logistic_company_get_custom_option('top_panel_image');
...
?>
		...
		<header class="top_panel_wrap top_panel_style_7 scheme_<?php
    echo esc_attr($post_options['scheme']); ?> top_panel_position_<?php
    echo esc_attr(logistic_company_get_custom_option('top_panel_position')); ?>">
			<div class="top_panel_wrap_inner top_panel_inner_style_7 top_panel_position_<?php
    echo esc_attr(logistic_company_get_custom_option('top_panel_position')); ?>">
			...
			<div class="top_panel_middle">
				<div class="content_wrap">
					<div class="contact_logo">
						<?php
    logistic_company_show_logo(true, true); ?>
					</div>
					<div class="menu_main_wrap">
						<nav class="menu_main_nav_area menu_hover_<?php
    echo esc_attr(logistic_company_get_theme_option('menu_hover')); ?>">
							<?php
...
?><div class="contact_field contact_phone">
                            <span class="contact_label contact_phone icon-icon_1"><?php
      echo '<a href="tel:' . preg_replace('/[\D]/', '', $contact_phone) . '">' . wp_kses_data($contact_phone) . '</a>'; ?></span>
                            <span class="contact_label contact_whatsapp icon-icon_1"><?php
      echo '<a href="https://api.whatsapp.com/send?phone=' . preg_replace('/[\D]/', '', $contact_fax) . '">' . wp_kses_data($contact_fax) . '</a>'; ?></span>
                            </div>
							<?php
      }
...
?>
                               <a href="<?php
        echo esc_url(wp_logout_url(home_url('/'))); ?>" class="popup_links icon icon-logout"><?php
        esc_html_e('Client Logout', 'logistic-company'); ?></a>
                            <?php
        }
      }
...
?>
    ...
    if (logistic_company_get_custom_option('show_page_image') == 'yes')
      {
?>
            <section class="top_panel_image" <?php
      logistic_company_show_layout($header_css); ?>>
                <div class="top_panel_image_hover"></div>
                <div class="top_panel_image_header">
                    <?php
      if (!empty($post_icon))
        { ?>
                        <div class="top_panel_image_icon <?php
        echo esc_attr($post_icon); ?>"></div>
                    <?php
        } ?>
                    <h1 itemprop="headline"
                        class="top_panel_image_title entry-title"><?php
      the_title(); ?></h1>

                    <div class="breadcrumbs">
                        <?php
      if (!is_404()) logistic_company_show_breadcrumbs(); ?>
                    </div>
                </div>
            </section>
        <?php
